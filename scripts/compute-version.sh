...
if [ "${1:-}" = "--current" ]; then
    LATEST=$(git -C "$REPO_ROOT" tag -l 'vice-mcp-*' --sort=-v:refname | head -1)
    if [ -z "$LATEST" ]; then
...
# Get upstream SVN revision from nearest r* tag
SVN_TAG=$(git -C "$REPO_ROOT" describe --tags --match 'r*' --abbrev=0 2>/dev/null || echo "")
if [ -z "$SVN_TAG" ]; then
    echo "ERROR: No r* tag found in git history" >&2
    exit 1
fi
SVN_REV="$SVN_TAG"
...
LATEST_TAG=$(git -C "$REPO_ROOT" tag -l "${PREFIX}-*" --sort=-v:refname | head -1)
